...
sealed class Action<S, A>

data class ChangeStateAction<S, A>(
...
data class ExternalWrappedAction<S, A>(internal val action: A) : Action<S, A>() {
    override fun toString(): String {
        return action.toString()
    }
}

class InitialStateAction<S, A> : Action<S, A>() {
    override fun toString(): String {
...
fun <S : Any, A> reducer(state: S, action: Action<S, A>): S =
    when (action) {
